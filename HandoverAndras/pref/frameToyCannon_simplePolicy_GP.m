...
updateSamples = 500;
controlContexts = 30;
absFeedbackFrequency = 5; % evalutes every #... sample
siga = .1;
...
saveFileName = ['PrefToyCannonSimpleGP',  '_AbsFreq', num2str(absFeedbackFrequency), '_InitNewSamples'...
...
for iter = 1:numIterations
    ...
    for iterPolicyUpdate = 1:policyUpdates
        ...
        loghyp = getFullGPModel([samples(fPrior(:, 1)), contexts(fPrior(:, 1))], fPrior(:, 2), -200);
...
%         drawnow
        ...
        [ypred, SigmaStar, KK] = predictWithFullGPModel(loghyp, [samples(fPrior(:, 1)), contexts(fPrior(:, 1))], fPrior(:, 2), sampUpdate);
        disp(['mean activation = ', num2str(mean(mean(KK /exp(loghyp(end-1))^2,1)))])
        SigmaStar = diag(SigmaStar);
