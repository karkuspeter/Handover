function [xsave] = sampleTrajectoryGPU2(train_x, train_y, px, hyp, ...
...
Broot = repmat(B.^.5, m*n, 1); 
Broot = reshape(Broot, d, dim*m*n)'; 
%C = repmat(C, m*n, 1); 
...
for i = 2:steps
	...
	ix_squared = parallel.gpu.GPUArray.false(m*n, 1);
	sqm = parallel.gpu.GPUArray.zeros(m*n, d);
	for k = 1:dim
		tmp2 = bsxfun(@times, tmp1', B(:, k).^.5)'; 
		ix_compute = all(abs(tmp2) <= 5, 2);
		sqm(~ix_squared & ix_compute, :) = tmp1(~ix_squared & ix_compute, :).^2;
		ix_squared = ix_squared | ix_compute;
		
		K = parallel.gpu.GPUArray.zeros(m*n, 1);
		K(ix_compute, :) = C(k)*exp(-.5*sqm(ix_compute, :)*B(:, k));
		K = reshape(K, m, n);
