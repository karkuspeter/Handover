function [xsave] = sampleTrajectoryGPU3single(train_x, train_y, px, hyp, ...
...
B = gpuArray(single(B));
C = gpuArray(single(C));
px_ext = gpuArray(single(px_ext));
x = gpuArray(single(x));
xsave = parallel.gpu.GPUArray.zeros(steps, d*n, 'single');
xsave(1, :) = x;
mean_factor = gpuArray(single(mean_factor));
var_factor = gpuArray(single(var_factor));
sigf_sq_add_var = gpuArray(single(sigf_sq_add_var));
sign_sq_add_var = gpuArray(single(sign_sq_add_var));
s = gpuArray(single(s));
polpar = gpuArray(single(polpar));
...
for i = 2:steps
    ...
    ds = means + parallel.gpu.GPUArray.randn(n, dim, 'single').*(vars.^.5)';
    s = s+ds;
