...
L_factor = gpuArray(L_factor);
Lm_factor = gpuArray(Lm_factor);
bet_factor = gpuArray(bet_factor);
...
for i = 2:steps
    ...
    lst = L_factor\K;
    lmst = Lm_factor\lst;
    means = (bet_factor*lmst)';
    vars = - sum(reshape(lst.^2, m, dim*n), 1) ... 
           + sign_sq_add_var.*sum(reshape(lmst.^2, m, dim*n), 1) ...
           + sigf_sq_add_var + sign_sq_add_var;
