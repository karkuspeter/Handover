function [f, g] = hyper_optim_GPUoptim(loghyp, input, target, verb)
...
Ky = K + parallel.gpu.GPUArray.eye(n)*signs*signs;
...
if 0
    ...
    while go
        try
            ...
            Ky = Ky + parallel.gpu.GPUArray.eye(n)*sig_sq_curr;
        end
    end
else
    L = chol(Ky);
...
if nargout > 1
    ...
    for i = 1:d
        Wloc = parallel.gpu.GPUArray.zeros(d); Wloc(i, i) = 1/w(i)/w(i)/w(i);
