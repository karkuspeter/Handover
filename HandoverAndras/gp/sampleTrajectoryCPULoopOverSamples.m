function [xsave] = sampleTrajectoryCPULoopOverSamples(train_x, train_y, px, hyp, ...
...
for i = 2:steps
    ...
    for j = 1:n
        lst(:, j) = L_factor\K(:, j);
        lmst(:, j) = Lm_factor\lst(:, j);
        means(j, :) = (bet_factor*lmst(:, j))';
        ix = (j-1)*dim+1:j*dim;
        vars(ix) = - sum(reshape(lst(:, j).^2, m, dim), 1) ... 
           + sign_sq_add_var(ix).*sum(reshape(lmst(:, j).^2, m, dim), 1) ...
           + sigf_sq_add_var(ix) + sign_sq_add_var(ix);
    end
