function [X, fX, i] = minimizeValidation(X, f, fv, length, varargin)
...
while i < abs(length)                                      % while not finished
  ...
  X0 = X; F0 = f0; dF0 = df0; Fv0 = f0;                  % make a copy of current values
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        [fv3] = feval(fv, rewrap(Z,X+x3*s));
        df3 = unwrap(df3);
        if isnan(fv3) || isinf(fv3) || isnan(f3) || isinf(f3) || any(isnan(df3)+isinf(df3)), error(''), end
        success = 1;
      catch                                % catch any error which occured in f
        x3 = (x2+x3)/2;                                  % bisect and try again
      end
    end
    if f3 < F0, X0 = X+x3*s; F0 = f3; dF0 = df3; Fv0 = fv3; end         % keep best values
    d3 = df3'*s;                                                    % new slope
  ...
  while (abs(d3) > -SIG*d0 || f3 > f0+x3*RHO*d0) && M > 0  % keep interpolating
    ...
    [fv3] = feval(fv, rewrap(Z,X+x3*s));
    df3 = unwrap(df3);
    if f3 < F0, X0 = X+x3*s; Fv0 = fv3; F0 = f3; dF0 = df3; end         % keep best values
    M = M - 1; i = i + (length<0);                             % count epochs?!
    d3 = df3'*s;                                                    % new slope
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    fvs = [fvs fv3];
%     disp([fs(:) fvs(:)])
    figure(11), plot([fs(:) fvs(:)]); legend('train set obj', 'validation set obj'), drawnow
%      fprintf('%s %6i;  Value %4.6e\r', S, i, f0);
    if exist('fflush','builtin') fflush(stdout); end
