function [X, fX, i] = minimizeGPU_checkGrad(X, f, length, input, target, varargin)
...
[f0 df0] = feval(f, X, varargin{:});          % get function value and gradient
    ...
    disp(['GPUs f: ', num2str(f0), ', df: ', num2str(df0(:)')])
    
    % first llhood and gradient (own)
    [ff, gh] = hyper_optim_GPUoptim_doubleOnly(X, input, target, 0);
    disp(['GPUd f: ', num2str(ff), ', df: ', num2str(gh(:)')])
    
 
    [ff, dff] = gp(hyp, @infExact, [], covfunc, likfunc, gather(input), gather(target));
    disp(['GPML f: ', num2str(ff), ', df: ', num2str([dff.cov', dff.lik])])
...
while i < abs(length)                                      % while not finished
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        dummy = rewrap(Z,X+x3*s);
        [f3 df3] = feval(f, dummy, varargin{:});
        
      
        disp(['GPUs f: ', num2str(f0), ', df: ', num2str(df0(:)')])
    
    % first llhood and gradient (own)
    [ff, gh] = hyper_optim_GPUoptim_doubleOnly(dummy, input, target, 0);
    disp(['GPUd f: ', num2str(ff), ', df: ', num2str(gh(:)')])
    
  hyp.cov = dummy(1:end-1);
    hyp.lik = dummy(end);
    [ff, dff] = gp(hyp, @infExact, [], covfunc, likfunc, gather(input), gather(target));
    disp(['GPML f: ', num2str(ff), ', df: ', num2str([dff.cov', dff.lik])])
        pause
  ...
  while (abs(d3) > -SIG*d0 || f3 > f0+x3*RHO*d0) && M > 0  % keep interpolating
    ...
    dummy = rewrap(Z,X+x3*s);
     hyp.cov = dummy(1:end-1);
    hyp.lik = dummy(end);
    disp(['GPUs f: ', num2str(f3), ', df: ', num2str(df3(:)')])
    
    % first llhood and gradient (own)
    [ff, hg] = hyper_optim_GPUoptim_doubleOnly(X, input, target, 0);
    disp(['GPUd f: ', num2str(ff), ', df: ', num2str(gh(:)')])
...
v = [];   
if isnumeric(s) || strcmp(class(s), 'parallel.gpu.GPUArray')
