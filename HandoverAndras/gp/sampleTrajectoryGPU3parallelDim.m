...
for i = 1:dim
	...
	Bs(:, :, i) = exp(hyp(1:end-2, i))';
	Cs(:, :, i) = exp(hyp(end-1, i));
	Mfs(:, :, i) = mean_factor(i, :)';
end
B = exp(hyp(1:end-2, :));
C = exp(hyp(end-1, :));
...
var_factor_cell = num2cell(var_factor, [1, 2]);
var_inarow = cat(3, var_factor(:, :));
...
for i = 2:steps
	...
	if 0
		dummy = cellfun(@(v, k) sum(bsxfun(@times, k, v*k), 1), var_factor_cell, num2cell(Kd, [1, 2]),  'UniformOutput' , false);
		dummy = squeeze(cat(3, dummy{:}));
		vars = bsxfun(@minus, Sigs + C , dummy);
	elseif 0
		vard = arrayfun(@(ind) Sigs(ind) + C(ind) - sum(bsxfun(@times, Kd(:, :, ind), var_factor(:, :, ind)*Kd(:, :, ind)), 1), 1:dim, 'UniformOutput', false);
		for k = 1:dim
			vars(:, k) = vard{k}';
		end
	else
		dummy = arrayfun(@(ind) reshape(dot(var_inarow, reshape(repmat(squeeze(Kd(:, ind, :)), m, 1), m, dim*m)), m, dim), 1:n, 'uni', false);
		vars = squeeze(sum(bsxfun(@times, Kd, permute(cat(3, dummy{:}), [1, 3, 2])), 1));
