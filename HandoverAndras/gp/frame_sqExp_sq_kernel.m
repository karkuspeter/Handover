...
% objfun = @(hyp) hyper_optim_GPUoptim_SqExp_Sq(hyp, xsamp(:), ysamp(:), 0, -1, 1);
% numGrad = numericalGradient(objfun, hyp_init(:));
hyp_init = log([.4, 1, 1, .1]);
[hyp_opt, fval, lines] = minimizeGPU(hyp_init, @(hyp) hyper_optim_GPUoptim_SqExp_Sq(hyp, xsamp(:), ysamp(:), 0, -1, 1, 0), -200);

hyp_init = log([.4, .4, 1, 1, .1]);
[hyp_opt, fval, lines] = minimizeGPU(hyp_init, @(hyp) hyper_optim_GPUoptim_SqExp_SqSqExp(hyp, xsamp(:), ysamp(:), 0, -1, 1, 0), -200);
% finish = toc;
