...
fixedW = kernelActivationTrick(data.samples, fixedActivation);
loghyp = log([0.5, 0.2]);
% get hyperparameters
options = optimoptions('fminunc', 'Algorithm','trust-region','GradObj','on','Hessian', 'off', 'MaxFunEvals', 1000, 'TolX', 1e-3, 'TolFun', 1e-2);
optfun = @(lh) pref_loghyp_numGrad_fixedKernelActivation(lh, data.samples, data.prefFeedback, data.absFeedback, ridge, 1, fixedW);
